...
import { seed, perlin3 } from '../../script/perlin'
...
export const Canvas = ()=>{
  ...
  const updateForces = (t) => {
    ...
    for (var x = 0; x < width; x+=20) {
      ...
      for (var y = 0; y < height; y+=20) {
        yOff += 0.1;
        let a = perlin3(xOff, yOff, t*0.00005) * Math.PI * 4;
  ...
  const launchParticle = () => {
    ...
    // particles[p].color = `hsl(${Math.floor(emitter.x/width*32)},70%,${50+Math.random()*20}%)`;
    particles[p].color = `	hsl(${171+p}, 98%, 53%)`;
    particles[p].alpha = 1;
    p++;
    if (p === nParticles) p = 0;
  }
...
return (
  <canvas ref={ref} className="absolute w-full h-screen z-0 pointer-events-none ">
